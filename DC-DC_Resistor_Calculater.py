...
def calculate_resistors(vref, vout_ref, standard):
    """计算最佳电阻组合"""
    error_min = 100.0
    best_r1 = 1000.0
    best_r2 = 1000.0
    ...
    series = RESISTOR_STANDARDS[standard]
    max_time = len(series)
    
    for i in range(max_time):
        for j in range(10):  # 电阻值范围：10^0 到 10^9
            r1_temp = series[i] * (10 ** j)
            for k in range(max_time):
                for l in range(10):  # 电阻值范围：10^0 到 10^9
                    r2_temp = series[k] * (10 ** l)
                    
                    # 计算输出电压
                    vout_temp = (r1_temp / r2_temp + 1) * vref
                    # 计算相对误差
                    error_temp = abs(vout_temp - vout_ref) / vout_ref
    ...
    # 实现C语言代码中的转换逻辑
    if best_r2 > 1000 or best_r1 > 1000:
        best_r1 /= 1000
        best_r2 /= 1000
